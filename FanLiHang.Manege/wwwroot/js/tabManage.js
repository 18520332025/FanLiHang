...
var int_admin_welcome = null;
var int_admin_department = null;
...
function reinitIframe(iframeId, minHeight) {
    try {
        ...
        bHeight += 3;
        if (bHeight > 10) {
            var intCook = iframeId.replace('if', 'int');
            eval('window.clearInterval(' + intCook + ')');
        }
...
function reloadTab() {
    var href = $("#tabList li.active a").attr('href').replace('#', '');
    $("#if_" + href).attr('src', $("#if_" + href).attr('src'));
    startIframeInit("if_" + href, 500);
}
...
function switchTab(option) {
    if ($('#tab_' + option.name).length == 0) {
        ...
        var content = '<div class="tab-pane active" id="' + option.name + '"><iframe src="' + option.url + '" scrolling="no" frameborder="0" id="if_' + option.name + '"  style="width:100%"></iframe></div>';
        $('#tab-content').append(content);
        int_department_index = startIframeInit("if_" + option.name, 500);
    }
